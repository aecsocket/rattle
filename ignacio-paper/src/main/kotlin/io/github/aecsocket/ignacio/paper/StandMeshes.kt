...
class StandMeshes internal constructor() {
    private val mMeshes = HashMap<UUID, BaseMesh>()
    val meshes: Map<UUID, StandMesh> get() = mMeshes

    operator fun get(id: UUID): StandMesh? = mMeshes[id]

    private fun create(
        transform: Transform,
        playerTracker: PlayerTracker,
        settings: StandMesh.Settings,
        mode: StandMesh.Mode,
        item: ItemStack
    ): StandMesh {
    ...
    fun createItem(
        ...
        settings: StandMesh.Settings,
        item: ItemStack
    ) = create(transform, playerTracker, settings, StandMesh.Mode.ITEM, item)

    fun createText(
        transform: Transform,
        playerTracker: PlayerTracker,
        settings: StandMesh.Settings
    ) = create(transform, playerTracker, settings, StandMesh.Mode.TEXT, air)

    fun remove(mesh: StandMesh, send: Boolean = true) {
        if (!TickThread.isTickThread())
    ...
    private sealed class BaseMesh(
        ...
        private fun computeYOffset() =
            baseYOffset + when (mode) {
                StandMesh.Mode.TEXT -> HEIGHT_TEXT
                StandMesh.Mode.ITEM -> if (small) HEIGHT_SMALL else 0.0
            }
