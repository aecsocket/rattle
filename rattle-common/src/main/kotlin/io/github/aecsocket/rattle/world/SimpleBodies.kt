...
/**
 * A subset of [Geometry] that a [SimpleBodies] can support. Since the simple-bodies engine also
 * handles showing bodies to players, we can't display *all* shapes. This is the subset that we can
 * support.
 */
...
abstract class SimpleBodies(
    ...
    // SAFETY: while a caller has access to a SimpleBodies object, they also have access to the
    // containing
  ...
  fun create(position: DIso3, desc: SimpleBodyDesc): ArenaKey {
    ...
    val collider =
        engine
            .createCollider(shape, Collider.Start.Relative())
            .material(desc.material)
            .mass(desc.mass)
            .let { physics.colliders.add(it) }
    val body =
        engine
            .createBody(desc.type, position)
    ...
    val (geomSettings, rawGeomScale) =
        when (val geom = desc.geom) {
  ...
  open fun removeAll() {
    instances.forEach { (_, instance) -> instance.destroy() }
    instances.clear()
  }
