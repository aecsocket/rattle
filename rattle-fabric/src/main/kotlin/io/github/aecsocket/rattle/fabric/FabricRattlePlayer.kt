...
class FabricRattlePlayer(
  ...
  // Sometimes, when a player is placed into a world, we get an error that
  // `ForwardingAudience$Single.audience()` is null (if we access `player.get(Identity.LOCALE)`
  // through Adventure's pointer mechanism)
  // Therefore, the Fabric Adventure platform has not initialized our player yet (for some reason).
  // So instead, we will initialize the messages with a default locale, and hope that the
  // PlayerLocales.CHANGED_EVENT changes to the player's actual locale later
