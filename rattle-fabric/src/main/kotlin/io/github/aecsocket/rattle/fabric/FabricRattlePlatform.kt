...
class FabricRattlePlatform(
  ...
  override fun physicsOrNull(world: World) = mod.physicsOrNull(world.unwrap())

  override fun physicsOrCreate(world: World) = mod.physicsOrCreate(world.unwrap())
  ...
  override fun onTick() {
    server.allLevels.forEach { level ->
      mod.physicsOrNull(level)?.withLock { physics -> physics.onTick() }
    }
    super.onTick()
  }
}
