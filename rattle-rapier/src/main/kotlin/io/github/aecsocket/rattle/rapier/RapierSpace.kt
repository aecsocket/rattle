...
internal constructor(
  ...
  val hooks =
      object : PhysicsHooks {
        override fun filterContactPair(context: PairFilterContext): Int {
          return onFilterContactPair
              .dispatch(
                  PhysicsSpace.OnFilterContactPair(
        ...
        override fun filterIntersectionPair(context: PairFilterContext): Boolean {
          return onFilterIntersectionPair
              .dispatch(
                  PhysicsSpace.OnFilterIntersectionPair(
                      colliderA = RapierColliderKey(context.collider1),
                      colliderB = RapierColliderKey(context.collider2),
                      bodyA = context.rigidBody1?.let { RapierRigidBodyKey(it) },
                      bodyB = context.rigidBody2?.let { RapierRigidBodyKey(it) },
        ...
        override fun modifySolverContacts(context: ContactModificationContext) {
          val res =
              onModifySolverContacts.dispatch(
                  PhysicsSpace.OnModifySolverContacts(
                      colliderA = RapierColliderKey(context.collider1),
                      colliderB = RapierColliderKey(context.collider2),
                      bodyA = context.rigidBody1?.let { RapierRigidBodyKey(it) },
                      bodyB = context.rigidBody2?.let { RapierRigidBodyKey(it) },
  ...
  private fun assignSpace(obj: RapierPhysicsNative) {
    obj.space?.let { existing ->
      throw IllegalStateException(
          "$obj is attempting to be added to $this but is already in $existing")
    }
    obj.space = this
  }

  override val colliders =
      object : PhysicsSpace.ColliderContainer {
  ...
  override val rigidBodies =
      object : PhysicsSpace.ActiveContainer<RigidBody, RigidBody.Mut, RigidBody.Own, RigidBodyKey> {
        override val count: Int
          get() {
            checkLock()
        ...
        override fun write(key: RigidBodyKey): RigidBody.Mut? {
          checkLock()
          key as RapierRigidBodyKey
          return rigidBodySet.getMut(key.handle)?.let {
            RapierRigidBody.Write(it, this@RapierSpace)
          }
        }
  ...
  override val impulseJoints =
      object : PhysicsSpace.ImpulseJointContainer {
        override val count: Int
          get() {
            checkLock()
        ...
        override fun read(key: ImpulseJointKey): ImpulseJoint? {
          ...
          return impulseJointSet.get(key.handle)?.let {
            RapierImpulseJoint.Read(it, this@RapierSpace)
          }
        }
        ...
        override fun write(key: ImpulseJointKey): ImpulseJoint.Mut? {
          checkLock()
          key as RapierImpulseJointKey
          return impulseJointSet.getMut(key.handle)?.let {
            RapierImpulseJoint.Write(it, this@RapierSpace)
          }
        }
        ...
        override fun add(
          ...
          assignSpace(value)
          return RapierImpulseJointKey(
              impulseJointSet.insert(bodyA.handle, bodyB.handle, value.handle, false))
        }
        ...
        override fun remove(key: ImpulseJointKey): Joint.Own? {
          ...
          return impulseJointSet
              ...
              ?.let {
                // `.remove` returns an ImpulseJoint.Mut, which contains some extra data relating to
                // that joint
                // we only want to return the `GenericJoint`, so we use a little hack and have
                // `.retainData()`,
                // a native function designed specifically to discard everything apart from the
                // GenericJoint
  ...
  override val multibodyJoints =
      object : PhysicsSpace.MultibodyJointContainer {
