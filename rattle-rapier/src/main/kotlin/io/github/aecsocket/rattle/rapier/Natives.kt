...
abstract class RapierRefCounted : RapierNative(), RefCounted {
  ...
  // equality and hashing is done by keying the underlying shape, **not** the ref-counting (Arc)
  // object
  override fun equals(other: Any?) =
      other is RapierRefCounted && handle.refData() == other.handle.refData()
...
fun VhacdSettings.toParams() =
    VHACDParameters(
        ...
        when (val fillMode = fillMode) {
          ...
          is VhacdSettings.FillMode.FloodFill ->
              VHACDParameters.FillMode.FloodFill(
...
fun rapier.dynamics.RigidBodyType.toRattle() =
    when (this) {
...
fun CoefficientCombineRule.toRattle() =
    when (this) {
...
fun JointAxis.Motor.Model.toRapier() =
    when (this) {
...
fun TOI.toRattle(): PhysicsQuery.Intersect =
    when (val status = status) {
      TOIStatus.PENETRATING -> PhysicsQuery.Intersect.Penetrating
      else ->
          PhysicsQuery.Intersect.Separated(
              state =
                  when (status) {
...
fun ComplexPointProject.toRattle() =
