...
private fun Array<out Dof>.bitMask() =
    (fold(0) { acc, layer -> acc or (1 shl layer.ordinal) }).toByte()
...
sealed class RapierImpulseJoint(
  ...
  internal constructor(
      ...
      RapierImpulseJoint(handle),
      ImpulseJoint.Mut,
      Joint.Mut by RapierJoint.Write(handle.data, space) {
    override val nativeType
      get() = "RapierJoint.Write"
