...
class RapierEngine internal constructor(var settings: Settings = Settings()) : PhysicsEngine {
  ...
  override fun createShape(geom: Geometry): Shape {
    val handle: SharedShape =
        when (geom) {
          ...
          is Box ->
              if (geom.margin > 0.0) {
                SharedShape.roundCuboid(
                    geom.halfExtent.x, geom.halfExtent.y, geom.halfExtent.z, geom.margin)
              } else {
          ...
          is ConvexDecomposition -> {
            ...
            val indices = geom.indices.map { intArrayOf(it.x, it.y, it.z) }.toTypedArray()
            if (geom.margin > 0.0)
                SharedShape.roundConvexDecomposition(
                    vertices,
                    indices,
                    geom.vhacd.toParams(),
                    geom.margin,
                ...
                SharedShape.convexDecomposition(
                    vertices,
                    indices,
                    geom.vhacd.toParams(),
                )
  ...
  override fun createBody(type: RigidBodyType, position: DIso3): RigidBody.Own {
    val body =
        RigidBodyBuilder.of(type.toRapier()).position(position.toIsometry()).use { it.build() }
  ...
  class Builder(
    ...
    override fun registerInteractionLayer(): InteractionLayer {
      ...
      return InteractionLayer.fromRaw(1 shl nextLayer).also { nextLayer += 1 }
    }
...
internal fun checkLock(objType: String, lock: ReentrantLock?) {
  ...
  if (!lock.isHeldByCurrentThread)
      throw IllegalStateException(
          "${Thread.currentThread().name}: Attempting to read/write $objType while not locked by this thread")
}
