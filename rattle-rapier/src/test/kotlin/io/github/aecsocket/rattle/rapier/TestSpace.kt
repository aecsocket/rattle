...
class TestSpace {
  ...
  fun testRefCount() {
    ...
    repeat(256) {
      val coll =
          engine
              .createCollider(shape.acquire(), Collider.Start.Relative())
  ...
  fun testLock() {
    ...
    assertThrows<IllegalStateException> { physics.colliders.count }

    lock.lock()
    val shape = engine.createShape(Sphere(0.5))
    val collKey =
        engine.createCollider(shape, Collider.Start.Absolute(DIso3.identity)).let {
          physics.colliders.add(it)
        }
    val coll = physics.colliders.read(collKey)!!
    lock.unlock()

    assertThrows<IllegalStateException> { coll.position }
  }
  ...
  fun testAddRemove() {
    ...
    val stepping =
        Thread {
              val dt = 1.0 / 60.0
              while (running.get()) {
                physics.withLock { physics -> engine.stepSpaces(dt, listOf(physics)) }
                ...
                val keys =
                    (0 until 50).map {
                      physics.withLock { physics ->
                        val collKey =
                            engine.createCollider(shape.acquire(), Collider.Start.Relative()).let {
                              physics.colliders.add(it)
                            }
                        val bodyKey =
                            engine.createBody(RigidBodyType.DYNAMIC, DIso3.identity).let {
                              physics.rigidBodies.add(it)
                            }
