...
/**
 * A physics structure which simulates dynamics - velocity, forces, friction, etc. - when attached
 * to a [PhysicsSpace]. A body may have [Collider]s attached to it, which provide the physical
 * collision shape used in contact response.
 *
 * # Body type
 *
 ...
 * - [RigidBodyType.DYNAMIC] - the body's position and velocity is managed by the physics engine, so
 *   it can be pushed by other colliders. The user can also specify forces to apply manually via
 *   [Mut.applyForce] and related methods.
 * - [RigidBodyType.KINEMATIC] - the body's position and velocity is managed by the physics engine,
 *   but the user manually sets a desired target position via [Mut.moveTo], and the engine
 *   calculates the required velocity to move it there.
 *
 * Note that not all operations can be applied to all body types - for example, [Mut.applyForce] on
 * a [RigidBodyType.FIXED] does not make sense. In these cases, the API will simply fail silently,
 * rather than throw an exception. It is your responsibility to make sure your bodies are of the
 * correct type before you call methods on them.
 *
 * # Continuous collision detection (CCD)
 *
 * Since a [PhysicsSpace] uses a discrete time-stepping approach, collisions may get missed if a
 * body is moving too fast. This is known as "tunneling", and it allows a body to effectively move
 * through another body without any collisions being detected. In this case, setting the fast-moving
 * body to have CCD enabled (using [Mut.isCcdEnabled]) will allow it to detect collisions via a
 * "sweep" between its old and new position, and find collisions in between. If there is a
 * collision, it will be teleported back to just before it collides that body, effectively
 * preventing tunneling. This works for both the linear and angular velocities.
 *
 * To check if a body is currently being simulated with CCD, use [isCcdActive].
 *
 * # Sleeping
 *
 * A body may not always be moving, so it would be a waste of resources to simulate its movement if
 * it has not moved much in the past few physics steps. This process is called sleeping, and a body
 * is automatically woken up if another body collides with it, or if woken up by the user.
 *
 * # Damping
 *
 * To simulate phenomena like air resistance, it would be too computationally expensive to actually
 * simulate fluid dynamics. Instead, we use a simple factor which the body's velocities are
 * multiplied by on every time step. This effectively slows the body down more the faster that it
 * moves. There are two damping factors:
...
interface RigidBody {
  ...
  interface Mut : RigidBody {
     ...
     *
     * **Note:** this is not a physically accurate action, as the body will effectively teleport
     * from one position to another without considering collisions in between. If you want to have
     * fine control over where the body moves, but have it still consider collisions in between,
     * consider using a [RigidBodyType.KINEMATIC] and [moveTo].
     *
    ...
    /**
     * For [RigidBodyType.DYNAMIC]: applies a (linear) force in Newtons at a specific position on
     * the body, taking into account rotation.
     */
    fun applyForceAt(force: DVec3, at: DVec3)

    /**
     * For [RigidBodyType.DYNAMIC]: applies a (linear) impulse - instantaneous change in force - at
    ...
    /**
     * For [RigidBodyType.DYNAMIC]: applies a (linear) impulse - instantaneous change in force - at
     * a specific position on the body, taking into account rotation.
     */
    ...
    /**
     * For [RigidBodyType.DYNAMIC]: applies an (angular) torque impulse - instantaneous change in
     * torque - causing a change in rotation.
     */
    fun applyTorqueImpulse(torqueImpulse: DVec3)

    /**
     * For [RigidBodyType.KINEMATIC]: sets this body's kinematic target to a specific position in
     * the world, and the physics engine will apply the correct velocities to move it to that
     * position. In this way, the movement is physically accurate (pushing other physics objects out
     * of the way) but this body will not be pushed by others.
     */
    fun moveTo(to: DIso3)
     ...
     *
     * @param strong If true, this body will be guaranteed to stay awake for at least a few physics
     *   steps. Otherwise, it might immediately go back to sleep if it still isn't moving enough.
     */
